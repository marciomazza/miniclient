...
fn body_wrapping() {
    ...
    assert_eq!(
        text(eval(
            &rt,
            "new DOMParser().parseFromString('<body><p>hi</p></body>', 'text/html').documentElement.outerHTML",
        )),
...
fn template_table_tags_in_content() {
    ...
    for (html, want) in [
        ...
        assert_eq!(text(eval(&rt, &src)), want, "{html}");
    }
}
...
fn insert_adjacent_html_table_context() {
    for (container, selector, markup, expected_selector) in [
        ...
        assert_eq!(
            json(eval(
                &rt,
                &format!(r#"document.querySelectorAll("{expected_selector}").length"#),
            ))
            .as_deref(),
            Some("1"),
            "{container}",
