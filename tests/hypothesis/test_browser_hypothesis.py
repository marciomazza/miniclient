...
def test_atob_btoa_round_trip(browser, s):
    encoded = browser.eval(f"btoa({_js_str(s)})")
    decoded = browser.eval(f"atob({_js_str(encoded)})")
    assert decoded == s
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(st.text())
def test_text_encoder_decoder_round_trip(browser, s):
    result = browser.eval(f"new TextDecoder().decode(new TextEncoder().encode({_js_str(s)}))")
    assert result == s
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(st.text())
def test_buffer_hex_round_trip(browser, s):
    hex_str = browser.eval(f"Buffer.from({_js_str(s)}, 'utf8').toString('hex')")
    result = browser.eval(f"Buffer.from({_js_str(hex_str)}, 'hex').toString('utf8')")
    assert result == s
...
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
@given(st.text())
def test_buffer_base64_round_trip(browser, s):
    b64 = browser.eval(f"Buffer.from({_js_str(s)}, 'utf8').toString('base64')")
    result = browser.eval(f"Buffer.from({_js_str(b64)}, 'base64').toString('utf8')")
    assert result == s
...
def test_url_search_params_round_trip(browser, params):
...
def test_buffer_concat_length(browser, a, b):
    js = f"""(() => {{
...
def test_url_search_params_size(browser, params):
...
def test_headers_append_has(browser, key, val1, val2):
