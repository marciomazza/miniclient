...
async def test_fetch_text_arbitrary_path(runtime_async, httpx_mock, path_segment):
    url = f"http://api.example.com/{path_segment}"
    httpx_mock.add_response(url=url, text="ok")
    result = await js_fetch_text(runtime_async, url)
    assert result == "ok"
...
async def test_fetch_with_query_params(runtime_async, httpx_mock, params):
...
async def test_fetch_status_arbitrary(runtime_async, httpx_mock, status_code):
...
async def test_fetch_text_arbitrary_body(runtime_async, httpx_mock, body):
...
async def test_fetch_json_arbitrary_dict(runtime_async, httpx_mock, data):
    url = "http://api.example.com/data"
    httpx_mock.add_response(url=url, json=data)
    result = await js_fetch_json(runtime_async, url)
    assert result == data
...
async def test_fetch_empty_or_binary_response(runtime_async, httpx_mock, binary_body):
...
async def test_fetch_sends_arbitrary_headers(runtime_async, httpx_mock, headers):
    url = "http://api.example.com/headers"
    httpx_mock.add_response(url=url, text="ok")
    await js_fetch_text(runtime_async, url, headers=headers)
...
async def test_fetch_receives_arbitrary_headers(runtime_async, httpx_mock, response_headers):
...
async def test_fetch_post_arbitrary_json_body(runtime_async, httpx_mock, payload):
    url = "http://api.example.com/echo"
