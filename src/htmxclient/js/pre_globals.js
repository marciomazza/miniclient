(function () {
    ...
    globalThis.performance ??= { now: () => Date.now(), mark: () => {}, measure: () => {}, getEntriesByType: () => [], getEntriesByName: () => [] };

    globalThis.setTimeout ??= (fn, d, ...a) => { try { fn(...a); } catch {} return 0; };
    globalThis.clearTimeout ??= () => {};
    globalThis.setInterval ??= (fn, d, ...a) => { try { fn(...a); } catch {} return 0; };
    ...
    globalThis.TextDecoder ??= class TextDecoder {
        ...
        decode(buf) {
            ...
            for (let i = 0; i < arr.length; ) {
                ...
                else if ((b & 0xe0) === 0xc0) { out += String.fromCharCode(((b & 0x1f) << 6) | (arr[i++] & 0x3f)); }
                else { out += String.fromCharCode(((b & 0x0f) << 12) | ((arr[i++] & 0x3f) << 6) | (arr[i++] & 0x3f)); }
            }
            return out;
        }
    };
