class URLSearchParams {
    ...
    delete(k) { this._params = this._params.filter(([p]) => p !== k); }
    get(k) { return this._params.find(([p]) => p === k)?.[1] ?? null; }
    getAll(k) { return this._params.filter(([p]) => p === k).map(([, v]) => v); }
    has(k) { return this._params.some(([p]) => p === k); }
    set(k, v) { const i = this._params.findIndex(([p]) => p === k); if (i < 0) this._params.push([k, v]); else { this._params[i] = [k, String(v)]; this._params = this._params.filter(([p], j) => p !== k || j === i); } }
...
class URL {
    constructor(url, base) {
        ...
        if (atIdx >= 0) { userinfo = authority.slice(0, atIdx); host = authority.slice(atIdx + 1); }
    ...
    set host(v) { const i = v.lastIndexOf(':'); if (i < 0) { this._hostname = v; this._port = ''; } else { this._hostname = v.slice(0, i); this._port = v.slice(i + 1); } }
    get pathname() { return this._pathname; }
    set pathname(v) { this._pathname = String(v); }
    get search() { return this._search; }
    set search(v) { this._search = v ? (v.startsWith('?') ? v : `?${v}`) : ''; this._sp = null; }
    ...
    get href() {
        const userinfo = this._username ? `${this._username}${this._password ? `:${this._password}` : ''}@` : '';
    ...
    static revokeObjectURL(id) { URL._blobs.delete(id); }
